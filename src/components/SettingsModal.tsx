...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const handleReindexEmbeddings = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to reindex embeddings');
    } finally {
                  ...
                  {activeTab === 'general' && (
                    <div className="space-y-5">
                      ...
                      <div className="space-y-1.5">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-stone-500 mt-0.5">
                                Move images from database to separate encrypted files. Creates thumbnails for faster loading.
                              </p>
                            </div>
                          </div>
                          ...
                          {migrationResult && (
                            <div className="mt-3 p-2 bg-emerald-50 border border-emerald-200 rounded-lg">
                              <p className="text-xs text-emerald-700">
                                ✓ Migrated {migrationResult.migrated} image{migrationResult.migrated !== 1 ? 's' : ''}
                        ...
                        <p className="text-xs text-stone-400">
                          Improves performance for vaults with many images. Safe to run multiple times.
                        </p>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="p-3 bg-stone-50 border border-stone-200 rounded-xl">
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-stone-500 mt-0.5">
                                Reindex all embeddings for faster semantic search. Run this after importing items.
                              </p>
                            </div>
                          </div>
                          ...
                          {reindexResult && (
                            <div className="mt-3 p-2 bg-emerald-50 border border-emerald-200 rounded-lg">
                              <p className="text-xs text-emerald-700">
                                ✓ Indexed {reindexResult.indexed} item{reindexResult.indexed !== 1 ? 's' : ''}
                                {reindexResult.failed > 0 && `, ${reindexResult.failed} failed`}
                                {reindexResult.skipped > 0 && `, ${reindexResult.skipped} skipped`}
                              </p>
                            </div>
                          )}
                        ...
                        <p className="text-xs text-stone-400">
                          Improves search speed by building a dedicated index. Safe to run multiple times.
                        </p>
                      </div>
                    </div>
                  )}
