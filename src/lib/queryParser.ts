import type { ParsedQuery, ParsedToken, DateRangeFilter, ItemType } from '../types';
...
const TIME_PATTERNS: TimePattern[] = [
  ...
  // "last week"
  {
    ...
    getRange: () => {
      ...
      return { start: startOfDay(start), end: endOfDay(end), label: 'last week' };
    },
  },
  // "this week"
  {
  ...
  {
    pattern: /^in\s+(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)$/i,
    getRange: (match) => {
      ...
      return { start: startOfDay(start), end: endOfDay(end), label: `in ${monthName}` };
    },
  },
  // Q1, Q2, Q3, Q4
  {
    ...
    getRange: (match) => {
      ...
      return { start: startOfDay(start), end: endOfDay(end), label: `Q${quarter}` };
    },
  },
  // "before:YYYY-MM-DD" or "before:YYYY/MM/DD"
  {
    ...
    getRange: (match) => {
      ...
      return { start: startOfDay(veryOld), end: endOfDay(date), label: `before ${dateStr}` };
    },
  },
  // "after:YYYY-MM-DD" or "after:YYYY/MM/DD"
  {
    ...
    getRange: (match) => {
      ...
      return { start: startOfDay(date), end: endOfDay(new Date()), label: `after ${dateStr}` };
    },
  },
  // Multi-word: "last X days"
  {
    ...
    getRange: (match) => {
      ...
      return { start: startOfDay(start), end: endOfDay(now), label: `last ${days} days` };
    },
  },
];
 ...
 */
export function parseQuery(query: string, existingTags: string[] = []): ParsedQuery {
 ...
 */
export function colorMatches(itemColors: string[], filterColors: string[], tolerance: number = 50): boolean {
...
export async function enhanceWithAI(
  ...
  try {
    ...
    const classificationMap = new Map(
      classifications.map(c => [c.token, c])
    );
    ...
    const enhancedTokens = parsedQuery.tokens.map(token => {
      ...
      if (!classification || !classification.filter_kind || classification.confidence < 0.7) {
        return token;
      }
      ...
      return {
        ...
        filterKind: classification.filter_kind as 'date' | 'type' | 'color' | 'tag',
      ...
      if (token.type === 'filter' && token.confidence < 1.0) {
        ...
        switch (token.filterKind) {
          case 'type':
            ...
            if (token.resolvedValue && !enhancedFilters.types.includes(token.resolvedValue as any)) {
              enhancedFilters.types.push(token.resolvedValue as any);
            }
            break;
          case 'color':
            enhancedFilters.colors = enhancedFilters.colors || [];
            if (token.resolvedValue && !enhancedFilters.colors.includes(token.resolvedValue)) {
              enhancedFilters.colors.push(token.resolvedValue);
            }
            break;
          case 'tag':
            enhancedFilters.tags = enhancedFilters.tags || [];
            if (token.resolvedValue && !enhancedFilters.tags.includes(token.resolvedValue)) {
    ...
    console.warn('AI classification failed, using deterministic results:', error);
    return parsedQuery;
  }
}
