...
const CODE_KEYWORDS = [
  ...
  'function', 'const', 'let', 'var', 'return', 'import', 'export', 'async', 'await',
  'class', 'extends', 'interface', 'type', 'enum', 'implements',
  // Python
  'def ', 'elif', 'lambda', 'from ', 'import ', 'self.', '__init__',
  // Rust
  'fn ', 'pub ', 'impl ', 'struct ', 'enum ', 'match ', 'mut ',
  // Go
  'func ', 'package ', 'go ', 'defer ', 'chan ',
  // Java/C#
...
function detectLanguage(content: string): string | null {
  ...
  if (/^import\s+.*\s+from\s+['"]/.test(trimmed) || /^export\s+(default\s+)?/.test(trimmed)) {
    if (trimmed.includes(': ') || /:\s*(string|number|boolean|any|void)/.test(trimmed)) return 'typescript';
    return 'javascript';
  }
  if (/^<\w+/.test(trimmed) && /<\/\w+>$/.test(trimmed)) return 'html';
  if (/<[A-Z]\w+/.test(trimmed) || /className=/.test(trimmed)) return 'jsx';
  if (/^\s*def\s+\w+\s*\(/.test(trimmed) || /^import\s+\w+$/.test(firstLine)) return 'python';
  if (/^\s*fn\s+\w+\s*\(/.test(trimmed) || /^use\s+\w+::/.test(trimmed)) return 'rust';
  if (/^package\s+\w+/.test(firstLine) || /^func\s+\w+/.test(trimmed)) return 'go';
  if (/^(public|private)\s+(static\s+)?(void|class|int|String)/.test(trimmed)) return 'java';
  if (/^#include\s*</.test(trimmed)) return 'cpp';
  if (/^SELECT\s+.+\s+FROM\s+/i.test(trimmed)) return 'sql';
  if (/^\s*{\s*"\w+":\s*/.test(trimmed) || /^\[\s*{/.test(trimmed)) return 'json';
  if (/^[.#]?\w+\s*{/.test(trimmed) && /:\s*.+;/.test(trimmed)) return 'css';
  if (/^\w+:/.test(firstLine) && (trimmed.includes('  - ') || trimmed.includes(': |'))) return 'yaml';
...
export function detectCode(content: string): CodeDetectionResult {
  ...
  const sentenceLikeLines = lines.filter(line => /^[A-Z].*[.!?]$/.test(line.trim()));
